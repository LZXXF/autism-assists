    ...
    <table class="table table-hover">
      ...
      <tbody>
          <tr v-for="(item, idx) in items" v-bind:key="item">
              ...
              <td> {{ item.doctor }} </td>
              <td> {{ item.active }} </td>
              <td><router-link :to = "'/PatientEventData1/' + item._id + '/' + item.firstname + '/' + item.lastname">View Events Data</router-link></td>
              <!--
              <td><router-link to = "/Params/123">params</router-link></td>
              <td><a :href = "'/#/PatientEventData1?patientId=' + item._id">View Events Data</a></td>
              -->
          </tr>
      </tbody>
    </table>
